...
Write-Host '== 3-a 파생물 생성(스캔 -> 라우터 인벤토리 -> 색인)'
# **연쇄다** — 뒤 단계가 앞 산출물을 읽는다(2026-09-22 실측: 인벤토리가 없어 UIS 재생성이 exit 1).
Push-Location $Ws
& node (Join-Path $Plugin 'scripts\scan_source.js') .
if ($LASTEXITCODE -ne 0) { Write-Warning '소스 스캔 실패 — 재생성·RECON이 막힙니다' } else { Write-Host '  OK source_index' }
& python (Join-Path $Plugin 'scripts\build_router_inventory.py') .
if ($LASTEXITCODE -ne 0) { Write-Warning '라우터 인벤토리 실패 — 화면 재생성이 막힙니다' } else { Write-Host '  OK screen_inventory' }
