...
> [반영: FUNC-order-004] SR-207로 신규 화면 `GET /member/{memberId}`(`MemberViewController`)가 이 API의
> 응답(`MemberService.get` 재사용, HTTP 재호출 없음)을 소비하도록 추가됐다 — 요청/응답/규칙/오류 계약은
> 변경 없음(AS-IS 유지, qa-agent round2 실측 확인). `screens` 필드는 아직 갱신하지 않았다 — 이 화면의
> UIS 신규 채번이 아직 안 됐고(STORY-FUNC-order-004 `## 후속 추적(TODO)` #2·#3), 존재하지 않는
> UIS-ID를 여기 채우면 참조가 끊긴다. UIS 채번 시 `screens: ["UIS-ORD-00X"]`로 갱신할 것.
