...
- 레거시 주의: `MEMBER_SIGNUP_VERIFICATIONS.requested_at`/`previous_requested_at`/`daily_count` 컬럼은 round3 잔재(deprecated) — round4부터 앱이 더 이상 쓰지 않는다(카운터는 `MEMBER_SIGNUP_RATE_LIMITS`로 완전히 이전됨).
- 재발송 시 시도 횟수·소비 표시 리셋(SR-295): 같은 채널·타깃 행에 재발송하면 새 6자리 코드·만료시각과 함께 `attempt_count=0`, `consumed_at=NULL`, `verified_at=NULL`로 함께 리셋한다 — 잠긴(`attempt_count>=5`) target이 재발송을 받으면 새 코드로 정상 검증 가능해진다(INF-MBR-002 `MBR-4093` 회복 경로). `attempt_count`/`consumed_at`/`verified_at`의 판정·소비는 여전히 INF-MBR-002(FUNC-member-003)가 소유한다 — 이 API는 재발송 시 초기화만 한다.
...
3. 판정을 통과한 경우에만 `MEMBER_SIGNUP_VERIFICATIONS`에 원자 UPSERT(`writeCode`)로 6자리 코드·만료시각을 기록한다(1단계와 별개의 autocommit 문 — 하나의 트랜잭션으로 묶여 있지 않다). 재발송(기존 행 갱신)이면 같은 문장에서 `attempt_count=0`, `consumed_at=NULL`, `verified_at=NULL`도 함께 리셋한다(SR-295) — 신규 행(INSERT)은 DDL 기본값이 이미 이 값들이라 리셋 대상이 없다
...
| 일자 | SR | 항목 | 변경 요약 | 커밋 |
|---|---|---|---|---|
| 2026-09-15 | SR-295 | #1 | 재발송(writeCode) 시 attempt_count/consumed_at/verified_at을 함께 리셋하도록 UPSERT SET절 확장 — MBR-4093 회복 경로 | shop-api@45e21e8 |
