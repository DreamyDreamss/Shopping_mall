---
story-id: STORY-SR-313.3
item: SR-313.3
title: GET /api/products/{sku} — 상품 단건 조회
status: Approved
domain: order
created: 2026-09-19
spec_markers: 0
sr-id: SR-313
approved_sha: fe301286532e
---

# STORY-SR-313.3 — 카탈로그 기반 스키마 — 카테고리 3단·브랜드·상품 이미지·상품정보고시 — GET /api/products/{sku} — 상품 단건 조회

## Story
카탈로그 기반 스키마 — 카테고리 3단·브랜드·상품 이미지·상품정보고시 — GET /api/products/{sku} — 상품 단건 조회
...
- **SR-313 #2** GET /api/products — 판매중 상품 목록 조회 — Approved

## 수용 기준 (Acceptance Criteria)
**변경(TO-BE) — 이 SR이 바꾸는 것** (`docs/변경관리/SR-313/02_변경명세.md`에서 도출)
- [ ] INF-ORD-009: 파라미터 `sku` 불변.
- [ ] INF-ORD-009: 응답에 `brand`·`categoryPath`·`images`·`notices`·`tags` 5개 필드 추가(기존 7필드 불변, 값 없으면 null/빈 배열).
- [ ] INF-ORD-009: `sale_yn` 무필터 동작 불변.
- [ ] INF-ORD-009: 인증(`SKU-` 접두사 공개 판정) 불변.
- [ ] INF-ORD-009: 오류(401/404) 계약 불변, 404 판정 기준(sku로 조회된 상품 없음) 불변, 새 오류 코드 없음.
- [ ] INF-ORD-009: 참조 테이블에 신규 5개 테이블 추가.

**회귀(AS-IS 유지) — 바뀌면 안 되는 것**
- [ ] INF-ORD-009 (GET /api/products/{sku}): 목록 조회([[INF-ORD-008]])와 달리 `sale_yn` 필터가 없다 — 판매종료 상품도 SKU를 알면 조회 가능
- [ ] INF-ORD-009 (GET /api/products/{sku}): [반영: SR-306] `listPrice`·`imageUrl`은 표시 전용 — 주문·장바구니 금액 계산은 종전대로 `price`만 쓴다.
